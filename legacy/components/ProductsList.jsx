...
const ProductCard = ({ product, index }) => {
  ...
  // Strip HTML and get a concise text summary (max 150 chars)
  const plainDescription = useMemo(() => {
    ...
    try {
      ...
      // Remove elements that are likely secondary details
      const elementsToRemove = doc.querySelectorAll('ul, ol, table, h1, h2, h3, h4, h5, h6, .specifications');
      elementsToRemove.forEach(el => el.remove());
      
      const textContent = doc.body.textContent || "";
      const cleanText = textContent.replace(/\s+/g, ' ').trim();
      
      // Truncate to 150 characters as requested
      return cleanText.substring(0, 150) + (cleanText.length > 150 ? '...' : '');
    } catch (e) {
      // Fallback regex strip
      return product.description.replace(/<[^>]*>?/gm, '').substring(0, 150) + '...';
    }
  }, [product.description]);
    ...
    >
      <Link to={`/product/${product.id}`}>
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm glass-card border-0 text-white overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
          ...
          <div className="p-4">
            <h3 className="text-lg font-bold truncate mb-2">{product.title}</h3>
            {/* Displaying simplified description (max 150 chars, no lists/bullets) */}
            <p className="text-sm text-gray-300 h-12 overflow-hidden leading-relaxed">
