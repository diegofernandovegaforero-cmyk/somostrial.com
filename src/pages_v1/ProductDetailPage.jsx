...
function ProductDetailPage() {
  ...
  // Clean description to remove bullet points, lists, specifications etc.
  // Returns only the first paragraph or clean text content to ensure conciseness.
  const cleanDescription = useMemo(() => {
    ...
    try {
      ...
      // Remove all lists (ul, ol), tables, headers, and specification blocks
      const elementsToRemove = doc.querySelectorAll('ul, ol, table, h1, h2, h3, h4, h5, h6, dl, .specifications, .tags, blockquote');
      ...
      if (firstParagraph) {
        ...
        // Fallback: get body text if no paragraphs are structured
      ...
      return product.description.replace(/<[^>]*>?/gm, '');
    }
  }, [product?.description]);
  ...
  return (
    <>
      <Helmet>
        <title>{product.title} - DIGITRIAL</title>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 glass-card p-8 rounded-2xl">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            ...
            {/* Display cleaned, simplified description text only (no HTML blocks) */}
            ...
            {product.variants.length > 1 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium text-white mb-2">{t('store.style')}</h3>
